...
export function loadExample(filename: string): FileStructure {
  ...
  lines.forEach((line) => {
    if (delimiter === '') {
      if (line[0] === line[1] && line[0] === line[2] && line.substring(4) === 'curvenote') {
        description = content.trim();
        delimiter = line.substring(0, 3);
        content = '';
        identifier = line.substring(4);
      } else {
        content = content.concat(line, '\n');
      }
    } else if (line.substring(0, 3) === delimiter) {
      if (identifier === 'curvenote') {
        curvenote = content;
      } else {
        output = { ...output, [identifier]: content.trim() };
      }
      delimiter = line.substring(0, 3);
      content = '';
      identifier = line.substring(4);
  ...
  output = { ...output, [identifier]: content.trim() };
  return { description, curvenote, ...output };
}
